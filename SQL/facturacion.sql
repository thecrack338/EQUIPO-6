...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 04-05-2025 a las 03:36:47
...
INSERT INTO `cliente` (`cedula`, `nombre_cliente`, `apellido`, `direccion`, `telefono`) VALUES
('1', 'fsedfg', 'dfgfgfg', NULL, NULL);
...
(1, 1, 1, 2, 7.00, 2.24, 16.24, 14.00),
(2, 1, 1, 2, 7.00, 0.00, 14.00, 14.00),
(3, 1, 1, 2, 7.00, 16.00, 14.00, 14.00),
(4, 1, 1, 20, 7.00, 22.40, 162.40, 140.00),
(9, 1, 1, 2, 7.00, 0.00, 14.00, 14.00),
(10, 1, 2, 2, 7.00, 2.24, 16.24, 14.00),
(11, 1, 2, 20, 14.00, 44.80, 324.80, 280.00);
...
CREATE TRIGGER `trg_calcular_iva_subtotal` BEFORE INSERT ON `detalle_venta` FOR EACH ROW BEGIN
    -- Obtener si el producto tiene IVA
    DECLARE producto_tiene_iva BOOLEAN;
    
    SELECT tiene_iva INTO producto_tiene_iva
    FROM productos
    ...
    -- Si el producto tiene IVA, calcular IVA y subtotal con IVA
    IF producto_tiene_iva THEN
        SET NEW.iva = NEW.subtotal * 0.16; -- 16%
    ELSE
        SET NEW.iva = 0;
    END IF;
...
INSERT INTO `productos` (`codigo_producto`, `precio`, `stock`, `nombre_producto`, `descripcion`, `categoria`, `tiene_iva`) VALUES
(1, 7.00, 10, 'hola', 'gfdgdf', NULL, 0),
(2, 7.00, 50, 'hola1', NULL, NULL, 1);
...
INSERT INTO `venta` (`codigo_venta`, `monto`, `fecha`, `cliente`) VALUES
(1, NULL, NULL, '1');
